...
train_loader = ImageFolder(r"D:\pycharm\face2\age\train",transform=transform)
test_loader = ImageFolder(r"D:\pycharm\face2\age\test",transform=transform)
...
class Net(nn.Module):
	def __init__(self):
		...
		self.conv1 = nn.Conv2d(3, 32, kernel_size=3)#2d卷积操作，用于处理二维图片
		self.bn1=nn.BatchNorm2d(32)#批量归一化
		self.conv2 = nn.Conv2d(32, 64, kernel_size=3)#卷积核大小为5
		self.bn2=nn.BatchNorm2d(64)#批量归一化
		self.conv3 = nn.Conv2d(64, 64, kernel_size=3)  # 卷积核大小为5
		self.bn3 = nn.BatchNorm2d(64)  # 批量归一化
		self.fc1 = nn.Linear(64*14*14, 512)#全连接层
		self.fc2 = nn.Linear(512, 128)#4分类
		self.fc3 = nn.Linear(128, 4)  # 4分类
	...
	def forward(self, x):#前向传播
		...
		x = self.mish(self.bn3(F.max_pool2d(self.conv3(x), 2)))  # 卷积>池化>批量归一化>激活
		print(x.shape)
		x = x.reshape(-1, 64*14*14)#改变张量形状以输入到全连接层内
